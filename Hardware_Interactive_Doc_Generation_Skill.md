...
*   **排版进阶 1：消除多子图垂直留白与文字遮挡 (Row-by-Row 水平流)**：
    ...
    *   **破局约束 (Anti-Occlusion Rules)**：
        ...
        2. **跨图连线全局声明**：**跨越不同 `subgraph` 之间的连线，必须写在所有 `subgraph` 结构体定义的外部（即全局最外层）**。严禁在一个 `subgraph` 内部写出指向另一个内部节点的连线。
        3. **严禁 Subgraph 互连 (Node-to-Node Only)**：**绝对禁止直接使用 `SubgraphA --> SubgraphB`**。这种语法会导致 Mermaid 的箭头（marker-end）渲染出现严重的错位、悬空或变形。跨图连线必须精准定位到具体的节点，例如 `Node_in_A -->|"文字"| Node_in_B`，以保证箭头完美贴合节点边框！
    *   **解决方案 (外竖内横隔离法 + 规范化连线)**：
